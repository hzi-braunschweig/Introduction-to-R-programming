...
## Line plot
It is often used to explore how a continuous variable changes over time. It provides information about trend.
...
`plot(x)` to plot line plot, just as scatter plot but with the option ` type = "l"`. The `plot()` function can be used to plot many other types of plot.
...
Example: Draw a line plot for max and min depth against date on the same figure using different colours.

`lines()` To draw additional lines on an existing plot. The equivalent for scatterplot is `points()`.
...
**Quick Exercise**: On the scatter plot of turnout Vs income plotted in previous section, increase the income by 300 and plot the new scatter plot on the existing plot. Use different colours.
...
Swapping a scatter plot for a line plot is as simple as swapping `geom_point` for `geom_line`
...
- [ggplot2 references manual](https://cloud.r-project.org/web/packages/ggplot2/ggplot2.pdf)
