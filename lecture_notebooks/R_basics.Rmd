...
Of course we can not learn everything about **R** via this lecture, since there are books filling the content of **R** and it is still being developed further. As mentioned in the beginning of this lecture the (longterm) best way to learn programming is by doing it and that also means learning new things on you own. To be able to do that we give some ways to look for help.
...
The Fibonacci numbers are a sequence of numbers that play a suprising role $F_{0} = 0$, $F_{1} = 1$ and $F_{n} = F_{n-2} + F_{n-1}$ for $n\geq2$.
